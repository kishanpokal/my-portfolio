...
export const AboutSection = () => {
  ...
  return (
    <section id="about" className="relative py-16 md:py-24 px-4 sm:px-6 lg:px-12 bg-transparent overflow-hidden">
      {/* Background */}
                ...
                <div className="min-h-[100px]">
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className="text-sm sm:text-base text-muted-foreground leading-relaxed space-y-4 text-left"
                    >
                      <p>{tabContent[activeTab]}</p>

                      {activeTab === 'personal' && (
                        <div className="mt-4 p-3 bg-primary/5 rounded-xl border border-primary/10 text-left">
                          ...
                          <ul className="text-sm space-y-2 text-muted-foreground">
                            ...
                            <li className="flex items-start gap-2">
                              <span className="w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0 mt-1.5" />
                              <span>Developing Maunam (Game Project)</span>
                            </li>
                          </ul>
                        </div>
                      )}
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {techStack.map((stack, index) => (
                  <motion.div
                    key={index}
                    className="bg-background/60 border border-border/40 rounded-xl p-4 transition-all duration-300 hover:border-primary/30 group text-left"
                  ...
                  >
                    ...
                    <div className="space-y-1.5">
                      {stack.items.map((item, itemIndex) => (
                        <div key={itemIndex} className="flex items-start gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors duration-300">
                          <div className="w-1.5 h-1.5 bg-primary/60 rounded-full flex-shrink-0 mt-1.5" />
                          <span>{item}</span>
                        </div>
                      ))}
                    </div>
          ...
          <div className="space-y-5">
            ...
            >
              ...
              <div className="flex gap-3 w-full">
                <motion.a
                  href="#contact"
                  className="flex-1 p-3 bg-primary text-primary-foreground rounded-xl text-center font-semibold transition-all duration-300 hover:bg-primary/90 group text-sm flex items-center justify-center gap-1.5"
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
                >
                ...
                <motion.button
                  ...
                  className="flex-1 p-3 border border-border/60 rounded-xl text-center font-semibold transition-all duration-300 hover:bg-accent hover:border-primary/30 group text-sm flex items-center justify-center gap-1.5"
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
                >
            ...
            >
              ...
              <div className="space-y-2">
                {features.map((feature, index) => (
                  <motion.div
                    key={index}
                    className="flex items-start gap-2.5 p-1.5 rounded-lg transition-all duration-300 hover:bg-background/60"
                  ...
                  >
                    ...
                    <span className="text-xs sm:text-sm text-muted-foreground text-left">{feature}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
