...
export const Navbar = () => {
  ...
  return (
    <>
      {/* Top Right Buttons */}
      <motion.div
        className="fixed top-24 right-4 md:top-4 md:right-4 z-40 flex flex-col md:flex-row gap-2"
      ...
      <motion.div
        className={cn(
          "fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-[95%] sm:w-auto",
      ...
      >
        <div className="flex items-center justify-center bg-white/80 dark:bg-black/80 backdrop-blur-md rounded-full shadow-lg p-2 border border-gray-200 dark:border-gray-700">
          <div className="flex items-center space-x-2 mr-4 md:mr-6 pl-2">
            <img src="/navbar-logo.svg" alt="Brand Logo" className="h-8 md:h-10 w-auto" />
          </div>
          <div className="flex space-x-1 items-center">
            {navItems.map((item) => (
              ...
              >
                ...
                <span className="text-xs sm:mx-2 hidden sm:block font-medium">{item.name}</span>
              </a>
            ))}
            <div className="flex items-center px-2 border-l border-gray-200 dark:border-gray-700 ml-1 pl-3">
              <ThemeToggle />
            </div>
          </div>
        </div>
